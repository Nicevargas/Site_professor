...
describe('resolução do endereço', () => {
  it('lê o identificador do caminho e do hash', () => {
    expect(slugFromRoute('/p/roberto-almeida', '')).toBe('roberto-almeida');
    expect(slugFromRoute('/', '#/p/roberto-almeida')).toBe('roberto-almeida');
    expect(slugFromRoute('/p/Roberto', '')).toBe('roberto');
    expect(slugFromRoute('/', '#/agenda')).toBeNull();
    expect(slugFromRoute('/', '')).toBeNull();
  });

  it('caminho funciona no domínio da plataforma', () => {
    expect(resolveTenant(loc(HOST, '/p/roberto-almeida'), HOST)).toEqual({ mode: 'path', slug: 'roberto-almeida' });
    expect(resolveTenant(loc(`www.${HOST}`, '/', '#/p/carlos'), HOST)).toEqual({ mode: 'path', slug: 'carlos' });
  });

  it('subdomínio de um nível vira professor', () => {
    expect(resolveTenant(loc(`roberto-almeida.${HOST}`), HOST)).toEqual({ mode: 'subdomain', slug: 'roberto-almeida' });
  });
  ...
  it('desenvolvimento e prévia não viram domínio de professor', () => {
    ...
    expect(resolveTenant(loc('localhost', '/', '#/p/carlos'), HOST)).toEqual({ mode: 'path', slug: 'carlos' });
  });

  it('sem domínio de plataforma configurado, só o caminho resolve', () => {
    expect(resolveTenant(loc('qualquer.coisa.com', '/'), '').mode).toBe('none');
    expect(resolveTenant(loc('qualquer.coisa.com', '/p/ana'), '')).toEqual({ mode: 'path', slug: 'ana' });
  });

  it('o endereço mais específico ganha do mais genérico', () => {
    // subdomínio presente e caminho presente: vence o subdomínio
    expect(resolveTenant(loc(`roberto.${HOST}`, '/p/carlos'), HOST)).toEqual({ mode: 'subdomain', slug: 'roberto' });
  });
});
