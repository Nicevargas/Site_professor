...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const vacationReturn = currentTeacher.vacationMode?.returnDate 
    ? formatVacationDateBR(currentTeacher.vacationMode.returnDate, true) 
    : (currentTeacher.vacationMode?.endDate ? formatVacationDateBR(currentTeacher.vacationMode.endDate, true) : 'em breve');
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[#f7f9fb] p-4 md:p-10 font-sans">
      <div className="max-w-7xl mx-auto space-y-8 pb-16">
          ...
          <div className="flex items-center space-x-3 w-full md:w-auto">
            {onOpenVacationModal && (
              <button
                ...
                className={`flex-1 md:flex-none h-12 px-4 rounded-lg font-semibold text-sm transition-all flex items-center justify-center border ${
                  isVacationActive
                    ? 'bg-amber-100 text-amber-900 border-amber-300 hover:bg-amber-200 shadow-xs'
                    : 'bg-[#e0e3e5] text-[#091426] border-[#c5c6cd] hover:bg-[#d8dadc]'
                }`}
                title="Configurar período de recesso ou ausência"
              >
                <Palmtree className={`w-4 h-4 mr-1.5 ${isVacationActive ? 'text-amber-700' : 'text-slate-600'}`} />
                <span>{isVacationActive ? 'Férias Ativas' : 'Modo de Férias'}</span>
              </button>
            )}
            <button 
        ...
        {isVacationActive && (
          <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-2xl p-4 md:p-5 text-white shadow-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-in fade-in">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-sm md:text-base">
                    Modo de Férias / Ausência Ativo
                  </h3>
                  <span className="px-2 py-0.5 bg-black/20 text-white rounded text-[10px] font-bold uppercase tracking-wider">
                    Agendamentos Pausados
                  </span>
                </div>
                <p className="text-xs text-amber-100 mt-0.5">
                  Novos horários online estão ocultos no seu site público até <strong>{vacationReturn}</strong>. Os alunos verão seu aviso de recesso.
                </p>
              </div>
            </div>
            ...
            {onOpenVacationModal && (
              <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
                <button
                  onClick={onOpenVacationModal}
                  className="flex-1 sm:flex-none px-4 py-2 bg-white text-amber-900 hover:bg-amber-50 font-bold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <span className="bg-emerald-100 text-emerald-900 px-2.5 py-0.5 rounded-full text-[11px] font-bold flex items-center gap-1">
                <span>Cobranças</span>
                <ChevronRight className="w-3 h-3" />
              </span>
            </div>
            <div>
              <p className="text-xs font-semibold text-[#45474c]">Faturamento & Cobranças</p>
