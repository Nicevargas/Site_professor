...
export const PublicLandingView: React.FC<PublicLandingViewProps> = ({
  ...
  // Segue a ordem escolhida pelo professor, e nao a ordem do catalogo
  const secoesDoMenu = secoesVisiveis
    .map((id) => SITE_SECTIONS.find((sec) => sec.id === id))
    .filter((sec): sec is (typeof SITE_SECTIONS)[number] => Boolean(sec));
  ...
  /**
   * Cada seção do corpo, pronta para ser posicionada.
   * Ficam num mapa, e não em sequência no JSX, porque a ordem quem manda
   * é o professor -- e menu e página precisam concordar.
   */
  const blocosDeSecao: Partial<Record<SiteSectionId, React.ReactNode>> = {
    curriculo: (
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb] text-[#191c1e] font-sans flex flex-col selection:bg-[#00687a] selection:text-white">
              ...
              {secoesDoMenu.map((sec) =>
                sec.id === 'faq' ? (
                  ...
                  >
                    ...
                    <span>{sectionLabel(sec.id, teacher.siteSectionLabels)}</span>
                  </a>
                ) : (
                  <a key={sec.id} href={`#${sec.id}`} className="hover:text-slate-900 py-2 transition-colors">
                    {sectionLabel(sec.id, teacher.siteSectionLabels)}
                  </a>
                )
              )}
            </nav>
      ...
      {/* As seções do corpo saem daqui, na ordem escolhida pelo professor.
          Antes elas ficavam em sequência fixa no JSX, então reordenar o menu
          faria o menu discordar da página. */}
      {secoesDoCorpo.map((id) => (
        <React.Fragment key={id}>{blocosDeSecao[id]}</React.Fragment>
      ))}
            ...
            {secoesDoMenu
              ...
              .map((sec) => (
                <a key={sec.id} href={`#${sec.id}`} className="hover:text-slate-900">
                  {sectionLabel(sec.id, teacher.siteSectionLabels, 'shortLabel')}
                </a>
              ))}
            {onOpenLogin && (
              <button
